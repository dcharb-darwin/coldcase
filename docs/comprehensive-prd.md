...
|---|---|---|
| 0.12.0 | 2026-05-20 | **Graph layer spine shipped.** Implements the design from 0.12.0-design. New `services/graph/` package: `types.py` (NodeKind / EdgeKind / GraphNode / GraphEdge with the four dimensions — confidence, provenance, temporal, trust / Subgraph / GraphPath / CrossCaseWitnessHit + closed-vocab confidence buckets), `interface.py` (GraphService Protocol — six methods covering the 5 query catalogue + invalidate + stats), `loader.py` (Mongo → graph for Case / Person / Document / Hypothesis / Tag / TimelineEntry with proper provenance + structural CO_OCCURS_WITH + loose-match SAME_NAME_AS + Jaccard SIMILAR_VIA_TAG), `networkx_backend.py` (in-memory MultiDiGraph with 60s TTL cache, thread-safe via coarse lock — typical build ~50ms for current demo data), `backend_factory.py` (env-var swap point for Mongo-native / Neo4j later). New `/graph/*` endpoints — stats, case neighborhood, persons across cases, persons network, persons shortest-path, cross-case role conflicts, cache invalidate. **Smoke verified live**: tenant graph 76 nodes / 119 edges across 31 cases / 20 persons / 15 docs / 2 hypotheses / 8 tags. Det. Mike Halberd → Dr. R. Hsieh resolves through `co_occurs_with` at 0.97 confidence. James M. Hinton spans CRRA-1945-CARTER and CC-2026-0001 as expected. Passage chunking, node-link visualization, Mongo-native + Neo4j backends, hot/warm/cold tier wiring all deferred per design doc — independently sized follow-ups |
| 0.12.0-design | 2026-05-20 | **Graph layer architecture locked.** Design doc landed at `docs/design/graph-layer.md`. Pragmatic interface-first design: `GraphService` Protocol with three swappable backends (`MongoNetworkXGraph` for small tenants — in-memory, default; `MongoNativeGraph` for medium scale via `$graphLookup` — no memory ceiling; `Neo4jGraph` read-replica for million-node-plus tenants — change-stream fed, Mongo stays system of record so §13663 hash chain is never at risk of divergence). Node kinds: Person, Case, Document, **Passage** (new — document chunking for passage-level addressability so a 2,000-page case file doesn't collapse to one node), Hypothesis, Tag, TimelineEvent. **Every edge carries four dimensions**: confidence (closed 5-bucket vocab from 0.20 speculative to 1.00 officer-confirmed), provenance (human / ai_generator / ai_red_team / derived_jaccard / derived_substring), temporal (valid_from / valid_to), trust (current / disputed / superseded). Hot/warm/cold tier model so big-case tenants don't load archived material at UI-render time. Tenant isolation non-negotiable at the interface (cross-tenant queries forbidden by type system). RBAC at edge resolution so cross-case neighborhood respects per-case read perms. Ranked 12-query catalogue covers daily-use detective workflow. **No code shipped in this entry** — implementation lands in 0.12.0 after sign-off |
| 0.11.0 | 2026-05-20 | **Multi-agent hypothesis shipped.** Implements the design locked in 0.11.0-design. Two new agents — `de_novo_generator` (case docs only, no brain dump) and `red_team` (attacks one hypothesis: counter-evidence + alternatives + bias flags from 9-slug closed vocab + logical gaps). Generator prompt revised to force at least one hypothesis the detective didn't raise. `Hypothesis.origin` enum (`human_typed` / `ai_from_braindump` / `ai_de_novo` / `ai_alternative`) with chip on every card; `parent_hypothesis_id` for alternative lineage; persisted `bias_flags` + `logical_gaps` + `red_team_count` accrue across red-team passes. Lifespan backfill upgrades pre-0.11 rows to the right origin. Endpoints: `POST /cases/{id}/hypotheses/generate`, `POST /cases/{id}/hypotheses/{id}/red-team`, `GET /hypothesis-bias-vocab`. Audit events: `HYPOTHESIS_GENERATED_DE_NOVO`, `HYPOTHESIS_RED_TEAMED`. UI: De-novo panel below the brain-dump composer, Challenge button + three-column challenge panel on every hypothesis card (counter-evidence with "Add to record", alternatives with "Investigate this instead", biases + gaps display-only with tooltips). `services/bias_vocab.py` holds the closed list. Verified live on the Marlene case — red-team surfaced confirmation bias + logical gap, alternative "Brenda Aragón misremembers the timing" accepted as a new hypothesis with parent linkage |
| 0.11.0-design | 2026-05-20 | **Multi-agent hypothesis design locked.** Design doc landed at `docs/design/hypothesis-agents.md`. Three agents survive review: `generator` (existing, prompt revised to "include at least one hypothesis the detective did not raise"), `de_novo_generator` (new — case docs only, no brain dump), `red_team` (new — given one hypothesis, produces counter-evidence + alternative explanations + bias flags from a closed 9-slug vocabulary + logical gaps; outputs are LLM suggestions, never auto-persisted). Rejected agents documented with reasons (no synthesizer — defeats the purpose; no prosecutor/defense — redundant with red-team; no pattern-matcher — handled by tag-Jaccard similar-cases; no steel-manner — wrong direction). New `Hypothesis.origin` enum (`human_typed` / `ai_from_braindump` / `ai_de_novo` / `ai_alternative`) so the detective sees at-a-glance whose framing they're looking at. Alternatives carry `parent_hypothesis_id` for lineage. Two new audit-event types: `HYPOTHESIS_GENERATED_DE_NOVO`, `HYPOTHESIS_RED_TEAMED`. **No code shipped in this entry** — implementation lands in 0.11.0 after design review |
